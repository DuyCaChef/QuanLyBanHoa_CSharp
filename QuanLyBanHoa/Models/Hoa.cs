...
using QuanLyBanHoa.Data; // ??m b?o b?n ?ã using namespace ch?a class Database
using System;
using System.Collections.Generic;
using System.Data;

namespace QuanLyBanHoa.Models // S?a namespace cho ?úng v?i th? m?c Models
{
    public class Hoa
    {
        ...
        public static bool Insert(Hoa hoa)
        {
            using (var conn = Database.GetConnection())
            {
                ...
                using (var cmd = new MySqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@MoTa", hoa.MoTa);
        ...
        public static List<Hoa> Search(string keyword)
        {
            ...
            using (var conn = Database.GetConnection())
            {
                ...
                string query = @"SELECT MaHoa, TenHoa, Gia, SoLuong, MoTa 
                               FROM hoa 
                               WHERE TenHoa LIKE @Keyword OR MoTa LIKE @Keyword OR MaHoa LIKE @Keyword 
                               ORDER BY MaHoa DESC";
                using (var cmd = new MySqlCommand(query, conn))
                {
