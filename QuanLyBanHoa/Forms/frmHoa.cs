    ...
    public partial class frmHoa : Form
    {
        ...
        private void SetupDataGridView()
        {
            if (dgDSHoa.Columns.Count > 0)
            {
                ...
                // Hide PhanLoai column if present
                if (dgDSHoa.Columns.Contains("PhanLoai"))
                {
                    dgDSHoa.Columns["PhanLoai"].Visible = false;
                }
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var hoaMoi = new Hoa { TenHoa = txtTenHoa.Text.Trim(), Gia = gia, SoLuong = soLuong, GhiChu = txtGhichu.Text.Trim() };
        ...
        private void SaveEdit()
        {
            ...
            try
            {
                var hoaCapNhat = new Hoa { MaHoa = currentMaHoa, TenHoa = txtTenHoa.Text.Trim(), Gia = gia, SoLuong = soLuong, GhiChu = txtGhichu.Text.Trim() };
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    if (ex.Number == 1451)
                        MessageBox.Show("Không thể xóa hoa này vì đã có đơn hàng liên quan!", "Lỗi ràng buộc dữ liệu", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    else MessageBox.Show("Lỗi khi xóa hoa: " + ex.Message, "Lỗi Database", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                catch (Exception ex)
                {
